...
The finalized 7,065-record concept artifact now drives the checked-in `docs/site/data/analysis/icml2026_people_topics.json`. The analysis is generated by `scripts/build_people_topics.mjs` during the normal site build.
...
2. **A stale people artifact could be combined with newer concepts.** Evidence: the browser requires matching concept fingerprint/count and index version/content hash before rendering it.
