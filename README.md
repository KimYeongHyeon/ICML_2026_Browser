...
The References view analyzes citation overlap from the current extracted-reference subset. It is not part of the first-page startup path and it does not require a backend.
...
- reference metadata is stored in a small manifest, a compact insight artifact, and lazy-loaded per-record shards,
- the References tab can show citation communities, bridge pairs, shared foundations, and selected-paper overlap evidence without slowing the first page load.
...
The checked-in artifact should be read as citation evidence for records with extracted references, not as a complete citation graph for all ICML records. Records without reachable PDFs, blocked sources, malformed PDFs, or parseable bibliography sections remain represented as unmatched rather than silently omitted.

The scheduled `Collect references` workflow is the repeatable update path: it chunks the whole index, extracts references from PDFs, merges the shards, validates the result, and commits improved public reference data as more PDFs become reachable.
